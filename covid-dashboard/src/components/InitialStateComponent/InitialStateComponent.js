  ...
  async function updateStatisctic() {
    ...
    await service
      ...
      .then(async () => {
        ...
        .then((response) => {
        state.countriesStat = [];
        const sortedArrayOfCoyntries = countriesStat.filter((item) => {
          return response.Countries.some((elem) => {
            return item.country === elem.Country;
          });
        });
        const sortedArrayOfStat = response.Countries.filter((item) => {
          return sortedArrayOfCoyntries.some((elem) => {
            return item.Country === elem.country;
          });
        });
